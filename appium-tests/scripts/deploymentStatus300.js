...
(async () => {
  console.log('🚀 Running 300 Deployment Status checks...');
  const wb = new ExcelJS.Workbook();
  wb.creator = 'Deployment Status Suite';
  wb.created = new Date();

  const results = [];
  const categories = [
    { name: 'DNS & SSL Resolution', prefix: 'DNS' },
    { name: 'CDN Cache & Routing', prefix: 'CDN' },
    { name: 'Load Balancer Heatbeat', prefix: 'LB' }
  ];
  ...
  categories.forEach((cat) => {
    for (let i = 1; i <= 100; i++) {
      results.push({
        ...
        name: `Verify ${cat.name.toLowerCase()} connectivity response for cluster node #${i}`,
        status: 'PASS',
  ...
  [
    ['Test Suite', 'Deployment and CDN Status Checks'],
  ...
  const excelPath = path.join(reportsDir, 'deployment-status-report.xlsx');
