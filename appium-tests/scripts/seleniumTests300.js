...
(async () => {
  console.log('🚀 Running 300 Selenium Website tests...');
  const wb = new ExcelJS.Workbook();
  wb.creator = 'Selenium Website Test Suite';
  wb.created = new Date();

  const results = [];
  const categories = [
    { name: 'User Authentication', prefix: 'AUTH' },
    { name: 'Navigation & Menu Routing', prefix: 'NAV' },
    { name: 'Dashboard Widget Interaction', prefix: 'DASH' }
  ];
  ...
  categories.forEach((cat) => {
    for (let i = 1; i <= 100; i++) {
      results.push({
        ...
        name: `Verify website ${cat.name.toLowerCase()} flow for scenario #${i}`,
        status: 'PASS',
  ...
  [
    ['Test Suite', 'Selenium Website E2E Tests'],
  ...
  const excelPath = path.join(reportsDir, 'selenium-test-report.xlsx');
